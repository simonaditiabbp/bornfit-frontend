...
export default function PTPlanEditPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg p-10 border border-gray-100">
      <h1 className="text-3xl font-bold mb-8 text-blue-700 border-b pb-3">PT Session Plan Detail</h1>      
      {editForm ? (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block font-medium mb-1">Name</label>
              <input type="text" className={`w-full p-3 border rounded-lg ${edit ? 'bg-white' : 'bg-gray-100'}`} value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))} required disabled={!edit} />
            </div>
            <div>
              <label className="block font-medium mb-1">Duration (days)</label>
              <input type="number" className={`w-full p-3 border rounded-lg ${edit ? 'bg-white' : 'bg-gray-100'}`} value={editForm.duration_value} onChange={e => setEditForm(f => ({ ...f, duration_value: Number(e.target.value) }))} required disabled={!edit} />
            </div>
            <div>
              <label className="block font-medium mb-1">Max Session</label>
              <input type="number" className={`w-full p-3 border rounded-lg ${edit ? 'bg-white' : 'bg-gray-100'}`} value={editForm.max_session} onChange={e => setEditForm(f => ({ ...f, max_session: Number(e.target.value) }))} required disabled={!edit} />
            </div>
            <div>
              <label className="block font-medium mb-1">Price (Rp)</label>
              <input type="number" className={`w-full p-3 border rounded-lg ${edit ? 'bg-white' : 'bg-gray-100'}`} value={editForm.price} onChange={e => setEditForm(f => ({ ...f, price: Number(e.target.value) }))} required disabled={!edit} />
            </div>
            <div>
              <label className="block font-medium mb-1">Minutes/Session</label>
              <input type="number" className={`w-full p-3 border rounded-lg ${edit ? 'bg-white' : 'bg-gray-100'}`} value={editForm.minutes_per_session} onChange={e => setEditForm(f => ({ ...f, minutes_per_session: Number(e.target.value) }))} required disabled={!edit} />
            </div>
            <div className="col-span-2">
              <label className="block font-medium mb-1">Description</label>
              <textarea className={`w-full p-3 border rounded-lg ${edit ? 'bg-white' : 'bg-gray-100'}`} value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))} required disabled={!edit} />
            </div>
          </div>
