...
export default function InsertMembershipSchedulePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-10 rounded-2xl shadow-lg mt-12 border border-gray-100">
      <h2 className="text-3xl font-bold mb-8 text-blue-700 text-center">Tambah Membership Schedule</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block font-medium text-gray-900 dark:text-white mb-1">Schedule Date <span className="text-red-600">*</span></label>
          <input type="date" name="schedule_date" value={form.schedule_date} onChange={handleChange} className="w-full border border-gray-300 p-2 rounded" required />
        </div>
        <div>
        ...
        <button type="button" className="bg-gray-400 text-white px-4 py-2 rounded ml-2" onClick={() => router.push('/admin/membership/schedules')}>Cancel</button>
      </form>
    </div>
  );
}
