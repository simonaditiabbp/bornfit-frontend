...
export default function ClassReportPage() {
  ...
  const handleDownload = async (type, format) => {
    ...
    try {
      const params = new URLSearchParams(Object.fromEntries(Object.entries(filters).filter(([_, v]) => v !== '')));
      const blob = await api.get(`/api/class-reports/${type}/${format}?${params}`, { responseType: 'blob' });
  ...
  return (
    <div>
      <PageBreadcrumb 
        items={[
          { icon: <FaDumbbell className="w-3 h-3" />, label: 'Class Reports' }
        ]}
      />
      ...
      <div className="m-5 p-5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg">
        ...
        <ReportDownloadSection 
          ...
          downloadOptions={[
            { label: 'Classes', type: 'classes' },
            { label: 'Attendance', type: 'attendance' },
            { label: 'Event Plans', type: 'event-plans' },
            { label: 'Instructor Performance', type: 'instructor-performance' }
          ]}
        ...
        <ReportFilterSection 
          ...
          onReset={() => setFilters({ start_date: '', end_date: '', class_type: '', search: '', page: 1, limit: 25 })}
        />
